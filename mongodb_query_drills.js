...
db.notes.find({}, {"content": 0, "_id": 0}).sort({"_id": 1});
...
db.notes.find({"_id": {$gt:ObjectId("000000000000000000000007")}});

// 11. Write a MongoDB query to display the documents from the collection notes which have an _id which is less than or equal to "000000000000000000000007".
db.notes.find({"_id": {$lte: ObjectId("000000000000000000000007")}});

// 12.  Write a MongoDB query to display the documents from the collection notes which have an _id which is greater than or equal to "000000000000000000000009" but less than or equal to "000000000000000000000017".
db.notes.find({"_id": {$gte: ObjectId("000000000000000000000009"), $lte: ObjectId("000000000000000000000017")}});
...
db.notes.updateOne({"_id":ObjectId("000000000000000000000003")}, {$set: {"title":'new title', "content":'new content'}});
...
db.notes.updateOne({"_id": ObjectId("000000000000000000000007")}, {$set:{"title": 'new title'}});
...
db.notes.updateMany({"_id": {$gt: ObjectId("000000000000000000000014")}}, {$set: {"title": 'new title', "content": 'new content'}});
...
db.notes.updateOne({"_id": ObjectId("000000000000000000000008")}, {$unset: {"title": 1}});
...
db.notes.updateMany({"_id": {$lte: ObjectId("000000000000000000000006")}}, {$unset: {"content": 1}});
...
db.notes.updateMany({"_id": {$lte: ObjectId("000000000000000000000003")}}, {$unset: {"title": 1}});
...
db.notes.deleteMany({"_id": {$gte: ObjectId("000000000000000000000013")}, "title": {$regex: /dogs/}});
